...
export default {
  ...
  methods: {
    ...
    closePage (event, name) {
      ...
      localStorage.pageOpenedList = JSON.stringify(this.$store.getters.pageOpenedList)
      if (this.currentPageName === name) {
        let lastPageName = ''
        if (this.$store.getters.pageOpenedList.length > 1) {
          lastPageName = this.$store.getters.pageOpenedList[1].name
        } else {
          lastPageName = this.$store.getters.pageOpenedList[0].name
        }
        this.$router.push({
          name: lastPageName
        })
