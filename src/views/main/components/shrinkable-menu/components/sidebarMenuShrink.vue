              ...
              <div style="text-align: center;" :key="index">
                  <Dropdown transfer v-if="item.children.length > 1" placement="right-start" :key="index" @on-click="changeMenu">
                      <Button style="width: 70px;margin-left: -5px;padding:10px 0;" type="text">
                          ...
                          <IconFont :size="20" :color="iconColor" :type="item.icon"></IconFont>
                      </Button>
                      <DropdownMenu style="width: 200px;" slot="list">
                          <template v-for="(child, i) in item.children">
                              <DropdownItem :name="child.name" :key="i">
                                <!-- <Icon :type="child.icon"></Icon> -->
                                <IconFont :type="child.icon"></IconFont>
                                <span style="padding-left:10px;">{{ itemTitle(child) }}</span>
                              </DropdownItem>
                          </template>
                    </DropdownMenu>
                </Dropdown>
                <Dropdown transfer v-else-if="item.children.length===1" placement="right-start" :key="index" @on-click="changeMenu">
