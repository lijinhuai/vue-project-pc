...
<template>
    <div class="main" :class="{'main-hide-text': hideMenuText}">
        <div class="sidebar-menu-con" :style="{width: hideMenuText?'60px':'200px', overflow: hideMenuText ? 'visible' : 'auto', background: $store.getters.menuTheme === 'dark'?'#495060':'white'}">
            <div class="logo-con">
...
export default {
  ...
  computed: {
    ...
    tagsList () {
      return this.$store.getters.tagsList // 所有页面的页面对象
    },
    pageTagsList () {
    ...
    menuIconColor () {
      return this.$store.getters.menuTheme === 'dark' ? 'white' : '#495060'
    },
    avatorPath () {
      return localStorage.avatorImgPath
    },
    cachePage () {
      return this.$store.getters.cachePage
    },
    lang () {
  ...
  created () {
    ...
    // 根据用户设置主题
    if (this.$store.getters.theme !== 'b') {
      let stylesheetPath = './static/' + this.$store.getters.theme + '.css'
      let themeLink = document.querySelector('link[name="theme"]')
